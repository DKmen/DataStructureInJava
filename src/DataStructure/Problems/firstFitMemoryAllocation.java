...
public class firstFitMemoryAllocation {
    ...
    public static void firstMemoryAllocationMethod(int[] memoryBlock,int[] processBlock){
        ...
        for(int i=0;i<processBlock.length;i++){
            for(int j=0;j<memoryBlock.length;j++){
                if(processBlock[i]<=memoryBlock[j]){
                    allocateProcess[i]=j+1;
...
//        System.out.println();
        ...
        firstMemoryAllocationMethod(memoryBlock,processBlock);
    }
}
